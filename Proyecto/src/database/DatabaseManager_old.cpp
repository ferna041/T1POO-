...
// Métodos adicionales para la tabla de reservas
...
bool DatabaseManager::cancelarReserva(int idReserva) {
    ...
    query.prepare("SELECT id_objeto FROM reservas WHERE id = :id");
    query.bindValue(":id", idReserva);
    ...
    if (query.next()) {
        ...
        qDebug() << "Reserva no encontrada con ID:" << idReserva;
        return false;
    }
    
    // Eliminar la reserva
    query.prepare("DELETE FROM reservas WHERE id = :id");
    query.bindValue(":id", idReserva);
